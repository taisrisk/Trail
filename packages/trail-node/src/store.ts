...
export type MailFolder = "inbox" | "priority" | "orders" | "finance" | "sent" | "archive" | "spam";
    ...
    try {
      ...
      if (result) {
        ...
        if (result.spamScore >= 7) {
           currentMail.folder = "spam";
        }

        // Flag importance
        if (result.importanceScore >= 8 && currentMail.folder !== "spam") {
           currentMail.status = "flagged";
        }

        // Notify user via OS native notifications if highly important or task triggered
        if (result.notifyUser && currentMail.folder !== "spam") {
          const { notifyDesktop } = await import("./outbound/notify");
          notifyDesktop("Trail AI Alert", result.notifyUser);
        }

        // Handle Rule Matches
        if (result.match && result.action) {
...
export async function configureSmtpOutbound(input: { host: string; port?: number; user: string; pass: string; dkimPrivateKey?: string; dkimPublicKey?: string; }) {
  const state = await readTrailState();
  if (state.vaultState === "locked") throw new Error("Vault is locked.");
  state.smtp = {
    host: input.host,
  ...
  await appendEvent(state, "connector.smtp", "Outbound SMTP credentials securely updated in local state.");
  return writeTrailState(state);
}
...
export async function configureDomainReceiver(input: { mode?: DomainReceiverConfig["mode"]; targetAddress?: string }) {
  ...
  state.receiver = { mode: input.mode || "cloudflare-email-routing", targetAddress: input.targetAddress || state.aliases[0]?.address || `inbox@${domain}`, webhookPath: "/api/ingress/relay", inboundSecret: process.env.TRAIL_INBOUND_SECRET || "default_inbound_secret_for_local_testing", status: "configured", updatedAt: now() };
  await appendEvent(state, "connector.receiver", `Domain receiver set to ${state.receiver.mode}.`);
  return writeTrailState(state);
}

export async function configureGmailConnector(input: { clientId: string; clientSecret: string; refreshToken: string; scopes?: string[] }) {
  const state = await readTrailState();
  if (state.vaultState === "locked") throw new Error("Vault is locked.");
  state.gmail = { clientId: input.clientId, clientSecret: input.clientSecret, refreshToken: input.refreshToken, scopes: input.scopes?.length ? input.scopes : ["gmail.readonly", "gmail.modify"], historyId: state.gmail?.historyId, syncState: "connected", imported: state.gmail?.imported || 0, lastScrapeAt: state.gmail?.lastScrapeAt, updatedAt: now() };
  await appendEvent(state, "connector.gmail", "Gmail OAuth connector configured with credentials.");
  return writeTrailState(state);
}
...
export async function scrapeGmailHistory(input: { limit?: number } = {}) {
  ...
  const clientSecret = state.gmail?.clientSecret;
  const refreshToken = state.gmail?.refreshToken;
  ...
  state = await readTrailState();
  if (state.gmail) { state.gmail.imported = (state.gmail.imported || 0) + count; state.gmail.syncState = "ready"; state.gmail.lastScrapeAt = now(); state.gmail.historyId = historyId; state.gmail.updatedAt = now(); }
